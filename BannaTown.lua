...
sections.AutoFarmSection:AddButton({
    enabled = true,
    text = "Next Target",
    flag = "AutoFarm_NextTarget",
    tooltip = "Skip current target and move to next criminal",
    callback = function()
        if _G.CurrentTarget then
            library:SendNotification("Skipping target: " .. _G.CurrentTarget.Name, 2, Color3.new(1, 1, 0))
            _G.SkipTarget = true
        end
    end
})

local function refreshCriminalsList()
    local criminals = {}
    for _, player in pairs(game:GetService("Players"):GetPlayers()) do
        if player ~= game.Players.LocalPlayer then
            local success = pcall(function()
                if player:FindFirstChild("RewardAfterWanted") then
...
local function refreshCriminalsList()
    ...
    task.spawn(function()
        for _, player in pairs(game:GetService("Players"):GetPlayers()) do
            if player ~= game.Players.LocalPlayer then
                local hasReward = pcall(function()
                    return player:FindFirstChild("RewardAfterWanted") or 
                           player:WaitForChild("RewardAfterWanted", 0.1)
                end)
                
                if hasReward and player.Character then
                    table.insert(criminals, player)
                end
            end
        end
...
sections.AutoFarmSection:AddButton({
    enabled = true,if IY_LOADED and not _G.IY_DEBUG == true then
...
task.spawn(function()
    ...
    text = "Refresh Criminals List",
    flag = "AutoFarm_RefreshCriminals",
    ...
    callback = function()
        ...
        if #criminals > 0 then
            ...
            for _, criminal in ipairs(criminals) do
                if criminal and criminal.Name then
                    criminalNames = criminalNames .. criminal.Name .. ", "
                end
            end
            ...
            if not _G.CurrentTarget then
                for _, criminal in ipairs(criminals) do
                    if not _G.ArrestedPlayers[criminal.Name] then
                        _G.CurrentTarget = criminal
                        library:SendNotification("New Target Set: " .. criminal.Name, 2, Color3.new(1, 1, 0))
                        break
                    end
                end
            end
        else
            library:SendNotification("No criminals found", 2, Color3.new(1, 1, 0))
        end
    end
})
...
local function startUnragdollLoop()
    ...
    _G.UnragdollLoop = true
    
    task.spawn(function()
        while _G.UnragdollLoop do
            local args = {"Unragdoll"}
...
sections.AutoFarmSection:AddToggle({
    ...
    callback = function(value)
        if value then
                    ...
                    if not _G.CurrentTarget then
                        ...
                        for _, player in pairs(game:GetService("Players"):GetPlayers()) do
                            if player ~= game.Players.LocalPlayer and 
                               not _G.ArrestedPlayers[player.Name] and
                               player:FindFirstChild("RewardAfterWanted") and
                        ...
                        if targetPlayer then
                            _G.CurrentTarget = targetPlayer
                            library:SendNotification("New Target: " .. targetPlayer.Name, 1, Color3.new(1, 0, 0))
                        end
                    end

                    if _G.CurrentTarget and _G.CurrentTarget.Character then
                        local handcuffs = game.Players.LocalPlayer.Backpack:FindFirstChild("Handcuffs")
                        if handcuffs then
                            handcuffs.Parent = game.Players.LocalPlayer.Character
                        end

                        if _G.CurrentTarget.Character:FindFirstChild("HumanoidRootPart") then
                            local targetCFrame = _G.CurrentTarget.Character.HumanoidRootPart.CFrame
                            local behindOffset = targetCFrame.lookVector * -3 
                            local arrestPosition = CFrame.new(
                                targetCFrame.Position - behindOffset,
                                targetCFrame.Position
                            )
                            
                            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = arrestPosition

                            local args = {_G.CurrentTarget}
                            local handcuffs = game.Players.LocalPlayer.Character:FindFirstChild("Handcuffs") or 
                                            game.Players.LocalPlayer.Backpack:FindFirstChild("Handcuffs")
                            
                            if handcuffs then
                                handcuffs.Parent = game.Players.LocalPlayer.Character
                                local success = handcuffs.RemoteFunction:InvokeServer(unpack(args))
                                
                                if success then
                                    _G.ArrestedPlayers[_G.CurrentTarget.Name] = true
                                    library:SendNotification("Arrested: " .. _G.CurrentTarget.Name, 2, Color3.new(0, 1, 0))
            ...
            _G.AutoArrest = false
            _G.CurrentTarget = nil
            library:SendNotification("Auto Arrest Disabled", 2, Color3.new(1, 0, 0))
        end
    end
})
...
local function setPenguinPrompt(state)
    local function findPenguinPrompt()
        local success, prompt = pcall(function()
            local model = workspace:GetChildren()[88]:GetChildren()[3]
            if model then
                if model:FindFirstChild("Sleeping") then
                    model.Sleeping.Value = false
                end
                return model["Mesh/Pad"].Attachment.ProximityPrompt
            end
        end)
        ...
        for _, model in ipairs(workspace:GetChildren()) do
            if model:IsA("Model") then
                ...
                local meshPad = model:FindFirstChild("Mesh/Pad", true)
                if meshPad and meshPad:FindFirstChild("Attachment") then
                    local foundPrompt = meshPad.Attachment:FindFirstChild("ProximityPrompt")
                    if foundPrompt then
                        return foundPrompt
                    end
                end
    ...
    if prompt then
        ...
        task.spawn(function()
            while state do
                prompt.Enabled = true
                local model = prompt:FindFirstAncestor("Model")
                if model and model:FindFirstChild("Sleeping") then
                    model.Sleeping.Value = false
                end
                task.wait(0.1)
            end
        ...
        library:SendNotification("Penguin Prompt Found and Enabled", 2, Color3.new(0, 1, 0))
    else
        library:SendNotification("Penguin Prompt Not Found - Retrying...", 2, Color3.new(1, 1, 0))
        task.spawn(function()
            task.wait(2)
            setPenguinPrompt(state)
        end)
    end
end
...
sections.MiscMain:AddToggle({
    ...
    tooltip = "Automatically prevents ragdoll effect",
    state = false,
    callback = function(state)
        if state then
            startUnragdollLoop()
