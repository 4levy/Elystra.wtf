...
VersionControl.check = function()
    if not CONFIG.CHECK_FOR_UPDATES then return true end
...
local function loadScript()
    ...
    if gameInfo then
        Notification.show(
            "Elystra.wtf",
            "Loading script for " .. gameInfo.name .. "...",
            3
        )
        ...
        if success then
            ...
            if loadSuccess then
                Notification.show(
                    "Elystra.wtf",
                    "Loaded successfully for " .. gameInfo.name,
                    5
                )
            else
                ...
                Notification.show(
                    "Error",
                    "Failed to execute script. Check console for details.",
                    8
                )
            end
        else
            ...
            Notification.show(
                "Error",
                "Failed to download script. Try again later.",
                5
            )
        end
    else
        ...
        Notification.show(
            "Elystra.wtf",
            "Sorry, " .. gameName .. " is not supported yet!",
            5
        )
...
if not success then
    ...
    pcall(function()
        Notification.show(
            "Critical Error",
            "Loader failed to run. Please report this issue.",
            10
        )
    end)
end
